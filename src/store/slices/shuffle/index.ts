...
export const getTarotResponse = createAsyncThunk(
  ...
  async (
    { question, tarot_id, speaker_id, category_id }: TarotRequest['request'],
    { rejectWithValue },
  ) => {
    try {
      const response = await shuffleApiService.getTarotResponse({
        tarot_id,
        question,
        speaker_id,
        category_id,
...
export const getTarotSpreads = createAsyncThunk(
  ...
  async (
    ...
    try {
      const response = await shuffleApiService.getTarotCards({
        params: { page: 1, per_page: 20, category_id: selectedCategory?.id },
      });
      const data = {
        data: response.data,
        lang,
...
export const shuffleSlice = createSlice({
  ...
  reducers: {
    ...
    setSelectedCategory: (
      state,
      action: PayloadAction<{ data: TarotCategory | null; lang: string }>,
    ) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        getTarotSpreads.fulfilled,
        (state, action: PayloadAction<{ data: TarotCard[]; lang: string }>) => {
