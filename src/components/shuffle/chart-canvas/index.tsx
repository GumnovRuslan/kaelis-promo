...
const getCardSize = () => {
  ...
  const width =
    minCardWidth +
    (maxCardWidth - minCardWidth) * ((clampedWidth - minWidth) / (maxWidth - minWidth));
  const height =
    minCardHeight +
    (maxCardHeight - minCardHeight) * ((clampedWidth - minWidth) / (maxWidth - minWidth));
...
export const ChartCanvas = ({ matrix, cards }: ChartCanvasProps) => {
  ...
  return (
    ...
    >
      <AnimatePresence>
        {selectedCard && <ModalCard card={selectedCard} onClose={handleCloseCard} />}
      </AnimatePresence>
    </div>
  );
};
  ...
  return (
    ...
    >
      <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
        ...
        <Image src={card.image} width={200} height={320} alt={card.label} priority />
        <h3>{card.label}</h3>
