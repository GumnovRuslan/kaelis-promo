...
export const shuffleApiService = {
  ...
  getTarotCategories: async (
    config?: AxiosRequestConfig,
  ): Promise<PaginatedResponse<TarotCategory>> => {
  ...
  getTarotResponse: async ({
    question,
    tarot_id,
    speaker_id,
    category_id,
  }: TarotRequest['request']): Promise<ApiResponse<TarotRequest['response']>> => {
    try {
      const response = await api.post('/tarot', {
        tarot_id,
        question,
        speaker_id,
        tarot_category_id: category_id,
      });
      return response.data;
    } catch (error: any) {
      throw error;
  ...
  getTarotAnswerFromChat: async (urlMessage: string): Promise<ApiResponse<TReadingResponse>> => {
